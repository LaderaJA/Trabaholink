...
def auto_verify_philsys(self, verification_id: int) -> Dict[str, Any]:
    ...
    try:
        ...
        # Check if portal is down/busy (network errors)
        if not philsys_result['success'] and philsys_result.get('retry_later'):
            logger.warning(f"PhilSys portal unavailable, will retry: {philsys_result.get('error')}")
            # Retry with exponential backoff
            raise self.retry(countdown=300)  # Retry after 5 minutes
        
        # If verification failed permanently (not a retry case)
        if not philsys_result['success']:
            logger.error(f"PhilSys verification failed permanently: {philsys_result.get('error')}")
            ...
            VerificationLog.objects.create(
                user=user,
                extracted_data={'philsys_web': philsys_result},
                process_type='auto',
                result='manual_review',
                notes=f"PhilSys auto-verification failed: {philsys_result.get('error')}"
            )
            
            return philsys_result
        
        # Extract verified data from portal
        verified_data = philsys_result.get('data', {})
        
        # Compare with user's form input
        match_result = compare_verification_data(user, verification, verified_data)
        
        # Save PhilSys data to log
        log = VerificationLog.objects.filter(user=user).order_by('-created_at').first()
        if log:
            log.extracted_data = log.extracted_data or {}
            log.extracted_data['philsys_web'] = philsys_result
            ...
            log = VerificationLog.objects.create(
                user=user,
                extracted_data={
                    'philsys_web': philsys_result,
                    'match_result': match_result
                },
                process_type='auto',
                result='pending',
                notes='PhilSys auto-verification completed'
            )
        
        # Auto-decision based on match
        if match_result['overall_match']:
            ...
            with transaction.atomic():
                ...
                Notification.objects.create(
                    user=user,
                    message="✅ Your PhilSys ID has been verified! Your account is now verified.",
            ...
            # AUTO-REJECT
            logger.info(f"Auto-rejecting verification {verification_id} - data mismatch")
            ...
            with transaction.atomic():
                ...
                Notification.objects.create(
                    user=user,
                    message=f"❌ PhilSys Auto-Verification Failed: The information on your ID does not match our government database records. Details: {match_result['mismatch_details']}. Please ensure your information is correct and submit a new verification request.",
